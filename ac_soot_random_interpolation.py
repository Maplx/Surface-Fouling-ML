...
# AC soot has a SINGLE impedance channel (no left/right split).
...
def build_dense_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    t_min = int(np.floor(t_ac.min()))
    t_max = int(np.ceil(t_ac.max()))
    t_grid = np.arange(t_min, t_max + 1, 1, dtype=float)

    z_dense = np.interp(t_grid, t_ac, z)
    temp_dense = np.interp(t_grid, t_temp, temp)
    ox_dense = np.interp(t_grid, t_co2, ox_rate)
    ...
    print(f"[INFO] Dense rows: {len(dense)}")
    print("\n[Dense preview]")
    print(dense.head(8))
...
res_path = os.path.join(OUT_DIR, "ac_soot_ox_random_interpolation_models.csv")
res_df.to_csv(res_path, index=False)

print("\n===== MODEL COMPARISON: AC Ox Rate (Dense) =====")
...
print("\n===== BEST MODEL RESULTS (AC Ox Rate Dense) =====")
print(f"MAE  = {mae:.6f}")
...
plt.xlabel("Time (s)")
plt.ylabel("Ox Rate")
plt.title(f"Dense AC: Ox Rate ({best_name_plot})")
plt.grid(True)
plt.legend()

fig_path = os.path.join(OUT_DIR, "ac_soot_ox_random_interpolation_best.png")
plt.tight_layout()
