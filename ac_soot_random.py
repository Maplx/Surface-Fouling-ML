...
def build_aligned_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    temp_aligned = np.interp(t_ac, t_temp, temp)
    ox_aligned = np.interp(t_ac, t_co2, ox_rate)
...
res_path = os.path.join(OUT_DIR, "ac_soot_ox_random_models.csv")
res_df.to_csv(res_path, index=False)

print("\n===== MODEL COMPARISON: AC Ox Rate (Aligned) =====")
...
print("\n===== BEST MODEL RESULTS (AC Ox Rate Aligned) =====")
print(f"MAE  = {mae:.6f}")
...
plt.xlabel("Time (s)")
plt.ylabel("Ox Rate")
plt.title(f"Aligned AC: Ox Rate ({best_name_plot})")
plt.grid(True)
plt.legend()

fig_path = os.path.join(OUT_DIR, "ac_soot_ox_random_best.png")
plt.tight_layout()
