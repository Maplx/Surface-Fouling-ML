...
res_path = os.path.join(OUT_DIR, "dc_soot_ox_random_interpolation_models.csv")
res_df.to_csv(res_path, index=False)

print("\n===== MODEL COMPARISON: DC Ox Rate (Dense) =====")
...
print("\n===== BEST MODEL RESULTS (DC Ox Rate Dense) =====")
print(f"MAE  = {mae:.6f}")
...
plt.xlabel("Time (s)")
plt.ylabel("Ox Rate")
plt.title(f"Dense DC: Ox Rate ({best_name_plot})")
plt.grid(True)
plt.legend()

fig_path = os.path.join(OUT_DIR, "dc_soot_ox_random_interpolation_best.png")
plt.tight_layout()
