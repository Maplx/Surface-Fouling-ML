...
def random_split(gas_df: pd.DataFrame, target_col: str, test_size: float = 0.5):
    """
...
# RIGHT target first
X_train, y_train, X_test, y_test, t_test = random_split(gas_df, "AC_right_aligned", test_size=0.5)
...
print("\n===== MODEL COMPARISON (RIGHT) - RANDOM 50/50 =====")
...
for i in range(preview):
    print(f"t={t_sorted[i]:.0f}  true={y_sorted[i]:.6f}  pred={pred_sorted[i]:.6f}")

# 6) Plot: True vs Predicted (test) over time
plt.figure()
plt.plot(t_sorted, y_sorted, label="True (test)", linewidth=2)
plt.plot(t_sorted, pred_sorted, label="Predicted (test)", linewidth=2)
plt.xlabel("t_gas_round (s)")
plt.ylabel("AC_right_aligned")
plt.title(f"Best Model Test Fit: {best_name}")
plt.grid(True)
plt.legend()
plt.show()
